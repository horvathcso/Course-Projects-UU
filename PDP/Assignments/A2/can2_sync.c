...
int main(int argc, char *argv[])
{
    ...
    if(shift!=dims[0]-1){
    MPI_Isend(A_loc, N_loc*N_loc, MPI_DOUBLE, left, 0, comm2D, &r_send_A);
    MPI_Irecv(buffer_A, N_loc*N_loc, MPI_DOUBLE,right, 0, comm2D, &r_recv_A);
    MPI_Isend(B_loc, N_loc*N_loc, MPI_DOUBLE, up, 0, comm2D, &r_send_B);
    MPI_Irecv(buffer_B, N_loc*N_loc, MPI_DOUBLE,down, 0, comm2D, &r_recv_B);
    }
    ...
    // Communication
    MPI_Wait(&r_send_A, MPI_STATUS_IGNORE);
    MPI_Wait(&r_send_B, MPI_STATUS_IGNORE);
    MPI_Wait(&r_recv_A, MPI_STATUS_IGNORE);
    MPI_Wait(&r_recv_B, MPI_STATUS_IGNORE);
    temp_A=A_loc;
    A_loc=buffer_A;
    buffer_A=temp_A;
    temp_B=B_loc;
