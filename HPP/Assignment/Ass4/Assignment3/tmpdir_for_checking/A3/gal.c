...
int main(int argc, char const *argv[]){
	...
	for(double t=0;t<=T;t+=dt){ //Evolve time
		...
		#pragma omp parallel for private(i,j,rij,C) shared(particles) num_threads(16)
		for(i=0;i<N;i++){ //For setting acceleration of ith particle 
			register double ax=0.0,ay=0.0;
			
			
			#pragma omp parallel for private(i,j,rij,C) shared(particles,ax,ay) num_threads(2)
			...
			particles[i].ux+=ax*dt+1;
			particles[i].uy+=ay*dt+1;}

		// #pragma omp parallel for private(i) shared(particles) num_threads(2) // makes performance much worse
		for(i=0;i<N;i++){
